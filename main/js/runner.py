...
def generate_func(func_name):
    def func(*args):
        ...
        try:
            response = requests.post('http://localhost:3000/', data=json.dumps(_input))
            if response.ok:
                try:
        ...
        process = run(['node', './main/js/src/main.js'], input=json.dumps(_input).encode(), stdout=PIPE, stderr=PIPE)
        result = None
